...
void c_typecheck_baset::typecheck_expr_operands(exprt &expr)
{
  ...
  {
    typecheck_code(to_code(to_unary_expr(expr).op()));
  }
  else if(expr.id()==ID_forall || expr.id()==ID_exists)
  {
    ansi_c_declarationt &declaration =
      to_ansi_c_declaration(to_binary_expr(expr).op0());
    ...
    to_binary_expr(expr).op0() = decl;

    typecheck_expr(to_binary_expr(expr).op1());
  }
  else
  {
...
{
  implicit_typecast_bool(to_binary_expr(expr).op0());
  implicit_typecast_bool(to_binary_expr(expr).op1());
